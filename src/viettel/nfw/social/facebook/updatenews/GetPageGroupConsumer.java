 ...
 * @author duongth5
 */
public class GetPageGroupConsumer implements Runnable {
    ...
    // log cua class
    private static final Logger LOG = LoggerFactory.getLogger(GetPageGroupConsumer.class);
    // session ket noi va lay msg tu activeMQ
    private Session session;
    // comsumer lay cac trang thai controller gui sang
    private final String consumerName = "collect_pg_consumer";
    private final String clientId = "clientId_collect_pg";
    private static ConcurrentHashMap<String, String> existedUrls = new ConcurrentHashMap<>();

    public GetPageGroupConsumer(String url) {
    ...
    public GetPageGroupConsumer(String url, String username, String password) {
        LOG.info("[StateConsumer] Start!");
        this.url = url;
                                    ...
                                    if (StringUtils.isNotEmpty(str)) {
                                        ...
                                        for (String part : parts) {
                                            if (existedUrls.containsKey(part)) {
                                                ...
                                                RunUpdateNews.storeObjToDb.getReceivedUrlsQueue().add(part);
